...
void update_fvmap(int id, int rate, int volt)
{
    ...
    for (i = 0; i < size; i++) {
        if (header[i].dvfs_type != id)
            continue;

        rv = _sram_base + header[i].o_ratevolt;
        for (j = 0; j < header[i].num_of_lv; j++) {
            if (rv->table[j].rate == rate) {
                rv->table[j].volt = volt;
                pr_info("update_fvmap: id=%d rate=%d => volt=%d\n", id, rate, volt);
                return;
            }
        }
    }
}
EXPORT_SYMBOL_GPL(update_fvmap);
...
void print_fvmap(void)
{
    ...
    for (i = 0; i < size; i++) {
        ...
        pr_info("FVMAP: %s [id=%x]\n", vclk->name, header[i].dvfs_type);
        rv = _sram_base + header[i].o_ratevolt;

        for (j = 0; j < header[i].num_of_lv; j++) {
            pr_info("  [%2d] %7d kHz = %d uV\n", j, rv->table[j].rate, rv->table[j].volt);
        }
    }
}
EXPORT_SYMBOL_GPL(print_fvmap);
